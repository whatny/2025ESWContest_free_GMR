...
void trilateration_3d(double d1, double d2, double d3, double d4, double &x, double &y, double &z) {
    ...
    const double x4 = 140.0, y4 = 0.0, z4 = 90.0;    // 앵커 4 (높이 추가)

    // 방정식 설정을 위한 행렬 구성
    // 이 방식은 선형 방정식 Ax=b 형태로 문제를 변환합니다.
    
    // 첫 번째 앵커를 기준점으로 사용하여 방정식 구성
    double A[3][3] = {
    ...
    // 행렬식이 너무 작으면 계산 불가능
    if (fabs(det) < 1e-6) {
        Serial.println("⚠️ Warning: det 값이 너무 작아 3D 계산 불가능!");
        x = -9999;
        y = -9999;
        z = -9999;
        return;
